...
public class MarkovText {
	 ...
	 * that the first two elements in the list are the String "<START>", and the
	 * last element of the list is the String "<END>". You want to update the map
	 * whatComesNext with information about which words come after each pair of
	 * words.
	 * 
	 * For each sequence of three adjacent words, do the following:
	 * <ul>
	 * <li>Create a sublist of the first two words. That will be your key for the
	 * map.
	 * <lI>If there isn't already a value for that key, you will want to put an
	 * empty list as the value for that key (just like with BuildIndex, where you'd
	 * put an empty Set as the value).
	 * <li>Take the value for that key, and add the third word to the end of the
	 * list.
	 * </ul>
	 */
	static void learnFromText(Map<List<String>, List<String>> whatComesNext, List<String> text) {
		...
		int x = 2;
		while (x < text.size()) {
			if (!whatComesNext.containsKey(text.subList(x - 2, x))) {
				whatComesNext.put(text.subList(x - 2, x), new ArrayList<String>());
			}

			whatComesNext.get(text.subList(x - 2, x)).add(text.get(x));
			x++;
		}
	...
	/**
	 * Return a word list generated by using the information in whatComesNext. The
	 * result should always start with the two occurrences of "<START>" and end with
	 * an occurrence of "<END>".
	 * 
	 * Then repeat the following process:
	 * <ul>
	 * <li>Use a sublist of the last two words in the result to get a list of
	 * possible next words from the map.
	...
	static ArrayList<String> generateText(Map<List<String>, List<String>> whatComesNext, Random r) {
		...
		while (notEnd) {
			...
			List<String> lookingFor = res.subList(x, x + 2);
			List<String> temp = whatComesNext.get(lookingFor);
			int rand = r.nextInt(temp.size());
			String toAdd = whatComesNext.get(lookingFor).get(rand);
	...
	/**
	 * given a list that starts with two occurrences of "<START>" and ends with the
	 * String "<END>", return the sublist that excludes those three elements.
	 */
	...
	/**
	 * Read all the words from a file and add them to a List. Where there is a blank
	 * line, add the String "\n" to the list.
	 */
	...
	/**
	 * print a word out to System.out, word wrapping if word would extend past 80
	 * characters. An empty string is treated as a new line.
	 */
